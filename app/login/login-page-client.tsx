...
export default function LoginPageClient({ initialUsers }: LoginPageClientProps) {
  ...
  const [recoveryMethod, setRecoveryMethod] = useState<"recoveryCode" | "adminPassword">("recoveryCode");
  ...
  async function resetPasswordWithRecovery(event: React.FormEvent) {
    event.preventDefault();
    if (!username || recoverySaving) return;

    const recoveryCode = recoveryForm.recoveryCode.trim();
    const adminPassword = recoveryForm.adminPassword.trim();
    const newPassword = recoveryForm.newPassword.trim();
    const confirmPassword = recoveryForm.confirmPassword.trim();

    if (recoveryMethod === "recoveryCode" && !recoveryCode) {
      setRecoveryError("请输入恢复码");
      return;
    }

    if (recoveryMethod === "adminPassword" && !adminPassword) {
      setRecoveryError("请输入管理员密码");
      return;
    }

    if (!newPassword) {
      setRecoveryError("请输入新密码");
      return;
    }

    if (newPassword.length < 4) {
      setRecoveryError("新密码至少需要 4 位");
      return;
    }
    ...
    const response = await fetch("/api/auth/recovery", {
      ...
      body: JSON.stringify({
        username,
        recoveryCode: recoveryMethod === "recoveryCode" ? recoveryCode : "",
        adminPassword: recoveryMethod === "adminPassword" ? adminPassword : "",
        newPassword,
      }),
    });
    ...
    if (!response.ok) {
      setRecoveryError(data.error || "重置密码失败");
      return;
    }

    setPassword(newPassword);
    setRecoveryForm({ recoveryCode: "", adminPassword: "", newPassword: "", confirmPassword: "" });
    setRecoveryError("");
    setPasswordNotice(data.message || "密码已重置，请直接登录");
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-5 py-10">
      <section className="w-full max-w-md rounded-[28px] border border-white/70 bg-white/75 p-6 shadow-soft backdrop-blur">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                onClick={() => {
                  ...
                  setRecoveryError("");
                  setPasswordNotice("");
                  setRecoveryMethod("recoveryCode");
                  setRecoveryForm({ recoveryCode: "", adminPassword: "", newPassword: "", confirmPassword: "" });
        ...
        <p className="mt-5 text-center text-xs text-stone-400">初始密码是 cat123；忘记密码前，请先在登录后的个人资料里生成恢复码并妥善保存。</p>
      </section>
      ...
      {recoveryDialogOpen ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-stone-900/20 p-4 backdrop-blur-sm">
          <form onSubmit={resetPasswordWithRecovery} className="w-full max-w-md rounded-[28px] bg-[#fbfaf8] shadow-soft">
            <header className="flex h-16 items-center justify-between border-b border-stone-200 px-5">
              <div>
                <h2 className="text-lg font-semibold">忘记密码</h2>
                <p className="text-xs text-stone-500">可使用恢复码，或由管理员密码协助重置当前用户密码</p>
              </div>
              <button
                type="button"
                onClick={() => {
            ...
            <div className="space-y-4 p-5">
              <div className="grid grid-cols-2 gap-2 rounded-2xl bg-stone-100 p-1">
                <button
                  ...
                  onClick={() => {
                     setRecoveryError("");
                     setRecoveryMethod("recoveryCode");
                     setRecoveryForm((previous) => ({ ...previous, adminPassword: "" }));
                   }}
                  className={`rounded-2xl px-3 py-2 text-sm font-medium transition ${
                    recoveryMethod === "recoveryCode" ? "bg-white text-stone-900 shadow-sm" : "text-stone-500"
                  }`}
                >
                  用恢复码
                </button>
                <button
                  ...
                  onClick={() => {
                     setRecoveryError("");
                     setRecoveryMethod("adminPassword");
                     setRecoveryForm((previous) => ({ ...previous, recoveryCode: "" }));
                   }}
                  className={`rounded-2xl px-3 py-2 text-sm font-medium transition ${
                    recoveryMethod === "adminPassword" ? "bg-white text-stone-900 shadow-sm" : "text-stone-500"
                  }`}
                >
                  用管理员密码
                </button>
              ...
              {recoveryMethod === "recoveryCode" ? (
                <label className="block">
                  <span className="mb-2 block text-sm font-medium text-stone-600">恢复码</span>
                  <div className="flex items-center gap-2 rounded-2xl border border-stone-200 bg-white px-4">
                    <KeyRound size={18} className="text-stone-400" />
                    <input
                      ...
                      onChange={(event) => {
                        setRecoveryError("");
                        setRecoveryForm((previous) => ({ ...previous, recoveryCode: event.target.value.toUpperCase() }));
                      }}
                      type="text"
                      autoCapitalize="characters"
                      className="h-12 flex-1 border-0 bg-transparent font-mono uppercase tracking-[0.2em] outline-none"
                    />
                  </div>
                  <p className="mt-2 text-xs text-stone-400">支持直接粘贴带短横线的恢复码。</p>
                </label>
              ) : (
                <label className="block">
                  <span className="mb-2 block text-sm font-medium text-stone-600">管理员密码</span>
                  <div className="flex items-center gap-2 rounded-2xl border border-stone-200 bg-white px-4">
                    <LockKeyhole size={18} className="text-stone-400" />
                    <input
                      value={recoveryForm.adminPassword}
                      onChange={(event) => {
                        setRecoveryError("");
                        setRecoveryForm((previous) => ({ ...previous, adminPassword: event.target.value }));
                      }}
                      type="password"
                      className="h-12 flex-1 border-0 bg-transparent outline-none"
                    />
                  </div>
                  <p className="mt-2 text-xs text-stone-400">仅在服务端配置了管理员重置密码时可用。</p>
                </label>
              )}
              <label className="block">
                <span className="mb-2 block text-sm font-medium text-stone-600">新密码</span>
                <div className="flex items-center gap-2 rounded-2xl border border-stone-200 bg-white px-4">
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      setRecoveryError("");
                      setRecoveryForm((previous) => ({ ...previous, newPassword: event.target.value }));
                    }}
                    type="password"
                    className="h-12 flex-1 border-0 bg-transparent outline-none"
                  />
              ...
              <label className="block">
                ...
                <div className="flex items-center gap-2 rounded-2xl border border-stone-200 bg-white px-4">
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      setRecoveryError("");
                      setRecoveryForm((previous) => ({ ...previous, confirmPassword: event.target.value }));
                    }}
                    type="password"
                    className="h-12 flex-1 border-0 bg-transparent outline-none"
                  />
              ...
              {recoveryError ? <p className="rounded-2xl bg-red-50 px-4 py-3 text-sm text-red-600">{recoveryError}</p> : null}
            </div>

            <footer className="flex gap-2 border-t border-stone-200 p-4">
              <button
                type="submit"
                disabled={recoverySaving || !username}
                className="flex h-12 flex-1 items-center justify-center rounded-2xl bg-ink text-sm font-semibold text-white disabled:opacity-50"
              >
                {recoverySaving ? "重置中..." : "确认重置"}
              </button>
              <button
                type="button"
                onClick={() => {
