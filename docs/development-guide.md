# 猫咪小家开发文档

## 1. 环境要求

- Node.js 22+
- npm 10+
- Windows PowerShell 可运行

## 2. 本地启动

```bash
npm install
npm run db:push
npm run db:seed
npm run dev
```

访问：

```text
http://127.0.0.1:3000
```

说明：仓库已在 `next.config.ts` 中固定 `turbopack.root` 为当前项目目录，避免上级目录存在额外 `package-lock.json` 时让开发服务错误解析 Tailwind 等依赖。

## 3. 默认账号

```text
月 / cat123
泽 / cat123
```

说明：当前版本的账号与猫咪都只保留名字；用户可在安全弹窗里生成恢复码，密码修改和恢复码重置都在登录页完成。登录页会读取数据库中的最新用户列表，并对连续输错密码做基础节流。
默认账号现在由 [prisma/seed.ts](prisma/seed.ts) 按固定 id 维护：`月 -> default-user-1`、`泽 -> default-user-2`。这样重复执行 seed 时不会因为名字变动而额外生成新的默认账号。

## 4. 环境变量

复制 `.env.example` 为 `.env`，或直接编辑现有 `.env`。

```bash
DATABASE_URL="file:./dev.db"
APP_SESSION_SECRET="local-dev-secret-change-before-production"
ADMIN_RESET_PASSWORD=""
LLM_BASE_URL="https://api.deepseek.com"
LLM_API_KEY=""
LLM_MODEL="deepseek-v4-pro"
TAVILY_API_KEY=""   # Pi Agent 联网搜索，从 app.tavily.com 获取
```

说明：开发环境未配置 `APP_SESSION_SECRET` 时会使用本地默认值；生产环境现在会直接报错并拒绝启动。`ADMIN_RESET_PASSWORD` 仅用于忘记密码时的管理员协助重置，不参与正常登录。默认账号密码会在 seed 或首次成功登录后写入为哈希值。
上传限制：聊天图片只接受 png、jpg、webp、gif，服务端会按文件头校验真实格式，不接受伪装扩展名或伪装 MIME 的文件。

DeepSeek 示例：

```bash
LLM_BASE_URL="https://api.deepseek.com"
LLM_API_KEY="你的 key"
LLM_MODEL="deepseek-v4-pro"
```

也支持使用 `DEEPSEEK_BASE_URL`、`DEEPSEEK_API_KEY`、`DEEPSEEK_MODEL` 这组变量名；未显式设置时，服务端会默认使用 DeepSeek 官方地址和 `deepseek-v4-pro`。

## 5. 常用命令

```bash
npm run dev      # 开发服务
npm run build    # 生产构建
npm run start    # 启动生产服务
npm run lint     # TypeScript 类型检查
npm run pi:cleanup-files # 清理 Pi Agent 过期/孤儿文件
npm run db:push  # 初始化/同步 SQLite 表
npm run db:seed  # 写入默认用户和猫咪
npm run db:reset # 重置数据库并重新 seed
```

## 6. 开发工作流

### 修改 UI

主要文件：

```text
components/chat-app.tsx        # 猫咪聊天主界面
components/agent/chat-ui.tsx   # Pi Agent 对话界面
app/globals.css
tailwind.config.ts
```

改完后执行：

```bash
npm run lint
npm run build
```

如果开发服务已运行，Windows 上 `npm run build` 可能因为 Prisma DLL 被占用而失败。先停止 dev server 后再 build。

### 修改 API

主要目录：

```text
app/api/           # 所有 API 路由
lib/               # 服务端逻辑（auth、llm、pi/...）
prisma/
```

修改 API 后至少验证：
- 未登录返回 401。
- 登录后正常返回。
- 错误输入有明确错误信息。
- 数据写入后刷新页面仍正确。
- 高频写接口达到阈值时应返回 429，并带明确提示文案。

### 修改数据库

当前本地初始化由 `prisma/init-db.ts` 负责。

如果修改 `prisma/schema.prisma`，需要同步修改：

```text
prisma/init-db.ts
prisma/seed.ts
```

然后执行：

```bash
npm run db:reset
```

## 7. 功能测试清单

### 登录

- `月` 可登录。
- `泽` 可登录。
- 未登录访问首页会跳转登录页。
- 登录页显示数据库中的最新用户名称。
- 同一来源对同一用户名连续输错 5 次后，第 6 次会返回限流错误；等待约 10 分钟后恢复。
- 当前节流状态会持久化到数据库里的 `AppSetting`，因此重启开发服务后锁定不会立即消失。

### 猫咪列表

- 默认 5 只猫展示正常。
- 选择猫咪后右侧聊天对象切换正常。

### 创建猫咪

- 点加号只打开创建弹窗，不立即创建。
- 取消创建后列表不增加猫咪。
- 创建时只填写名字。
- 创建成功后直接回到聊天界面并选中新猫。
- 重名时提示换名。

### 编辑猫咪

- 设置抽屉允许修改猫咪名字、性格、回复方式和补充设定。
- 新建猫咪后这些设定默认为空，保存后会影响后续回复风格。

### 账号安全

- 账号安全弹窗可以查看恢复码状态并生成新的恢复码。
- 账号安全弹窗可以设置“所有猫咪通用”的用户称呼。
- 当前版本不再支持修改头像和个人简介。

### 修改密码

- 修改密码入口位于登录页。
- 需要先选择用户，再填写当前密码、新密码和确认新密码。
- 修改成功后可直接用新密码登录。
- 修改密码后，旧登录态应立即失效，需要重新登录。
- 个人资料弹窗不再承担改密码职责。

### 忘记密码恢复码

- 登录后可在个人资料弹窗生成恢复码。
- 恢复码明文只会展示一次，重新生成后旧恢复码失效。
- 登录页可为当前选中用户输入恢复码并重置密码。
- 若 `.env` 配置了 `ADMIN_RESET_PASSWORD`，登录页也可切换为输入管理员密码来重置当前用户密码。
- 恢复成功后，旧登录态会立即失效，且该恢复码不可再次使用。
- 同一来源对同一用户名连续输错 5 次恢复码后，第 6 次会返回限流错误；等待约 10 分钟后恢复。

### 删除猫咪

- 删除入口位于危险操作区。
- 输入错误名称不能删除。
- 输入正确名称才能删除。
- 输入前后空格不影响确认。
- 删除后列表移除该猫。
- 删除后相关消息和历史设定残留会被级联清理。
- 不能删除到一只猫都不剩。

### 聊天

- 用户消息能保存。
- 用户可给猫咪发送图片。
- 猫咪回复能保存。
- 用户 1 发的消息，用户 2 可见。
- 若给用户设置了所有猫咪通用的称呼，回复里会优先使用该称呼。
- 若给猫咪填写了性格 / 回复方式 / 补充设定，回复风格会随之变化。
- 未配置 LLM 时返回备用回复。
- 配置 LLM 后调用真实模型。
- 同一用户 1 分钟内连续发送超过 12 次消息或图片时，应返回 429 提示稍后再试。

### 猫咪猫圈 / 电子相册

- 给猫咪发送图片后，会生成一条用户图片消息和一条猫咪回复。
- 同一张图片会自动进入该猫咪的猫圈列表。
- 电子相册应以按时间倒序的照片墙展示所有照片，并弱化文字，只保留时间信息。
- 点击任意照片后，应能打开以照片为主的沉浸式大图预览，并支持上一张 / 下一张切换。

### 悄悄话留言卡

- 可以正常创建、送达、查看和删除悄悄话。
- 同一用户 10 分钟内连续创建超过 6 张悄悄话时，应返回 429 提示稍后再试。
- 删除单条猫圈时，对应上传图片文件也应一并清理。
- 删除猫咪时，相关猫圈图片会一起清理。

### Pi Agent（`/agent`）

- 未登录直接访问 `/agent`，应跳转 `/login`。
- 登录后访问 `/agent`，页面正常渲染，侧边栏有「New Chat」和会话列表。
- 发送消息后，消息气泡出现在右侧，Assistant 回复从左侧流式渲染。
- 调用工具时，消息下方显示“处理思路”摘要卡片，而不是原始命令明细。
- 点击「停止生成」按钮后，流式输出立即停止，按钮切换回发送状态。
- 新建多个会话，切换会话后，消息历史独立、互不干扰。
- 删除会话（垃圾桶图标）后，会话从侧边栏移除。
- 上传文件默认保留 24 小时，生成文件默认保留 7 天；执行 `npm run pi:cleanup-files` 后，过期文件和孤儿文件应被清理。
- 点击侧边栏「←」按钮，返回猫咪聊天首页。
- 猫咪聊天主界面侧边栏用户信息区应显示 Bot 图标按钮，点击跳转 `/agent`。

## 8. 代码约定

- TypeScript 开启 `strict`。
- 手写 UI 优先保持苹果风、淡色、紧凑。
- Agent UI 独立暗色风格，参考 ChatGPT。
- 图标使用 `lucide-react`。
- 危险操作必须二次确认。
- 不把 API key 暴露到前端。
- 新增持久化字段时同时更新文档和 seed。
- Pi Agent 工具使用 TypeBox `Type` 描述参数（从 `@earendil-works/pi-ai` 引入，不单独安装 `@sinclair/typebox`）。

## 9. 已知注意事项

- 当前密码已改为哈希存储；旧明文数据会在成功登录后自动迁移。
- 当前 SQLite 适合本地开发，不适合多人高并发生产。
- 当前用户与猫咪的旧头像/个性字段仍保留在数据库结构中，但交互层已停用，只保留名字。
- 当前没有自动化测试框架，依赖手动/API 冒烟测试。
- Pi Agent 会话存储在进程内存，服务重启后全部丢失——当前是 MVP 行为，不是 bug。
- Pi Agent 的 `bash` 工具没有命令白名单，仅适合本地受信环境使用，不应暴露到公网。
- Pi Agent 上传/生成文件默认存放在 `tmp/pi/`，依赖 `AgentFile` 元数据表和 `npm run pi:cleanup-files` 做定期清理。
- `@earendil-works/pi-ai` 重导出了 TypeBox 的 `Type`，工具参数定义应从该包引入，不要单独安装 `@sinclair/typebox`（版本可能不兼容）。

## 10. 上线前检查清单

### 环境与部署

- 确认生产环境已设置 `APP_SESSION_SECRET`，且为独立随机长字符串。
- 确认生产环境已设置 `DATABASE_URL`，并验证数据库文件或挂载卷具备可写权限。
- 如果需要真实模型回复，确认 `LLM_BASE_URL`、`LLM_API_KEY`、`LLM_MODEL` 都已配置。
- 如果部署环境会重建容器或覆盖工作目录，确认 `public/uploads/` 已迁移到持久化存储或对象存储。
- 在目标部署环境执行一次 `npm run build`，不要只在本地开发机验证。

### 账号与安全

- 使用 `月`、`泽` 各登录一次，确认登录、退出登录、首页鉴权跳转都正常。
- 连续输错密码 6 次，确认第 6 次返回限流提示；等待窗口过去后可恢复登录。
- 修改任一账号密码后，确认旧登录态立即失效，刷新页面会回到登录页。
- 确认未登录直接访问 API 时会返回 401，而不是 200 或空数据。

### 关键功能

- 发送纯文本消息，确认用户消息、猫咪回复、刷新后历史记录都正常。
- 发送图片消息，确认图片写入成功、猫咪回复成功、对应内容进入猫圈。
- 删除单条猫圈，确认界面移除且对应上传图片文件被清理。
- 调用已停用的用户头像、猫咪头像、专属称呼接口时，应返回明确提示，不应再写入新数据。
- 创建、送达、回复、删除悄悄话各走一遍，确认双方视角都符合当前规则。

### 限流与异常

- 同一用户 1 分钟内连续发送超过 12 次消息或图片，确认接口返回 429。
- 同一用户 10 分钟内连续创建超过 6 张悄悄话，确认接口返回 429。
- 在未配置 LLM 的环境下发送消息，确认会回退到本地备用回复而不是报 500。
- 手动删除或失效 `cat_session` 后刷新页面，确认前端能正常回到登录页。

### 建议留档

- 记录生产环境实际使用的构建命令、启动命令、环境变量清单。
- 记录默认账号是否保留、初始密码是否已轮换。
- 记录上传文件的持久化方案和备份方案。
